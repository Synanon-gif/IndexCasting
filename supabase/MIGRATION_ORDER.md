...
165. `migrations/20260408_admin_security_hardening.sql`   ← Admin Security Audit fixes: HR-1 (VULN-09) p_role validation in admin_update_profile_full blocks role='admin' assignment; HR-2 replaces weak is_admin column check with assert_is_admin() UUID+email pin; M-1 drops stale triggers trg_prevent_admin_flag_escalation + trg_prevent_is_admin_escalation; M-2 fixes admin_update_model_all + admin_update_org_all RLS to use is_current_user_admin(); M-3 idempotently removes admin_full_access_overrides FOR ALL from admin_overrides. Run after #164.
