-- =============================================================================
-- Restore canonical get_guest_link_models (no org paywall / can_access_platform).
--
-- Live drift or manual changes could introduce has_platform_access() /
-- can_access_platform() inside this SECURITY DEFINER RPC. The function owner
-- may lack EXECUTE on can_access_platform → 42501 for authenticated callers.
--
-- Product: guest/package model loading is gated by link id, rate limit,
-- guest_links validity window, and H-4 (m.agency_id = link.agency_id) — not
-- by client org subscription.
--
-- Post-deploy verification (run on Live):
--   SELECT pg_get_functiondef(p.oid)
--   FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
--   WHERE n.nspname = 'public' AND p.proname = 'get_guest_link_models' AND p.pronargs = 1;
--   -- Must NOT contain can_access_platform or has_platform_access
-- =============================================================================
...
COMMENT ON FUNCTION public.get_guest_link_models(UUID) IS
  ...
  '20260530: canonical restore (effective_city, 7-day window, guest_link_access_log).';
...
BEGIN
  ...
  ), 'FAIL: get_guest_link_models missing after 20260530 migration';
END $$;
