-- =============================================================================
-- Discovery Audit C (2026-04-08): dedupe get_models_near_location per model_id
--
-- Issue: LEFT JOIN model_agency_territories mat ON mat.model_id = m.id without
-- narrowing produces N rows per model when N territories exist — duplicate cards
-- in Near Me and skewed pagination.
-- Fix: DISTINCT ON (m.id) with deterministic tie-break (country_code, agency_id).
-- Location priority (live > current > agency) remains in resolved_locations CTE.
-- =============================================================================
...
BEGIN
  ...
  deduped AS (
    ...
    ORDER BY
      j.id,
      j.territory_country_code NULLS LAST,
      j.territory_agency_id NULLS LAST
  )
  ...
  FROM (
    SELECT *
    FROM deduped d
    ORDER BY d.distance_km ASC, d.name ASC
    OFFSET p_from
    LIMIT  (p_to - p_from + 1)
  ) result;
END;
...
COMMENT ON FUNCTION public.get_models_near_location IS
  ...
  'Location: DISTINCT ON (model_id) live>current>agency. '
  'Territory metadata: one row per model (deduped join). Updated 20260408.';
