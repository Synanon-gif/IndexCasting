-- =============================================================================
-- add_model_to_project — territory-aligned agency for connection guard (2026-05-24)
--
-- Discovery (get_discovery_models) resolves the representing agency via
-- model_agency_territories for the browse country. The RPC previously checked
-- client_agency_connections only against models.agency_id, causing 400 when those
-- differ. We now resolve the effective agency as COALESCE(territory agency for
-- p_country_iso, models.agency_id). Optional p_country_iso preserves legacy
-- behaviour when NULL.
--
-- Consolidates overloads into a single function:
--   add_model_to_project(uuid, uuid, uuid, text) with defaults on last two params.
-- =============================================================================

DROP FUNCTION IF EXISTS public.add_model_to_project(uuid, uuid);
DROP FUNCTION IF EXISTS public.add_model_to_project(uuid, uuid, uuid);
...
COMMENT ON FUNCTION public.add_model_to_project(UUID, UUID, UUID, TEXT) IS
  'Adds a model to a client project. Pass p_organization_id when known; p_country_iso aligns agency with discovery (model_agency_territories).';
