-- =============================================================================
-- Discovery: exclude models with active option/casting requests for this client org
--
-- Builds on 20260826 (city filter + proximity). Adds NOT EXISTS on option_requests:
-- hard-exclude when same client org has a non-terminal option or casting row for
-- the model (status <> rejected, final_status <> job_confirmed).
-- Caller org guard unchanged (p_client_org_id membership verified before query).
-- =============================================================================
  ...
  filtered AS (
    ...
    WHERE
      ...
      AND NOT EXISTS (
        ...
        WHERE oq.model_id = m.id
          AND COALESCE(oq.client_organization_id, oq.organization_id) = p_client_org_id
          AND COALESCE(oq.request_type, 'option') IN ('option', 'casting')
          AND oq.status IS DISTINCT FROM 'rejected'
          AND COALESCE(oq.final_status, '') IS DISTINCT FROM 'job_confirmed'
      )
...
COMMENT ON FUNCTION public.get_discovery_models(
  ...
  'Ranked client discovery. 20261329: exclude active option/casting requests for '
  'p_client_org_id; 20260826 city/proximity tiers unchanged.';
...
BEGIN
  ...
  ), 'get_discovery_models missing after 20261329 migration';
END;
$$;
