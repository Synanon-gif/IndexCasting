-- =============================================================================
-- GDPR: export_user_data — Phase 2 (Art. 15/20 complete lifecycle export)
-- Replaces public.export_user_data only. No RLS / auth / billing changes.
-- Adds: messages received, conversations, recruiting chat, calendar_entries,
--       notifications, activity_logs, legal_acceptances, push_tokens,
--       expanded option_requests scope, structured `domains` + export_version.
-- =============================================================================
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'calendar_events', (
      ...
      FROM (
        SELECT id, title, start_date, end_date, event_type, owner_id, organization_id, created_at
...
COMMENT ON FUNCTION public.export_user_data(UUID) IS
  ...
  'Includes messaging, recruiting, calendar_entries, notifications, activity_logs, legal_acceptances, push_tokens. '
