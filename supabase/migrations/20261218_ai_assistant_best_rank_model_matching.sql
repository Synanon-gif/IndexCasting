-- =============================================================================
-- AI Assistant model matching: best-rank-only ambiguity hardening
--
-- Security boundary:
--   - No new intent and no new returned fields.
--   - Agency org and MAT scope are resolved before any ranking or fuzzy matching.
--   - Exact/full-token matches win over partial/token/fuzzy matches.
--   - If more than one visible model shares the best rank, all best-rank
--     candidates are returned for explicit clarification; no silent auto-select.
-- =============================================================================
...
BEGIN
  ...
  ORDER BY c.match_similarity DESC, c.scoped_display_name ASC;

  RAISE LOG 'ai_assistant_model_matching_threshold search_len=%, threshold=%',
    length(v_search_folded), v_similarity_threshold;
END;
$$;
...
COMMENT ON FUNCTION public.ai_read_model_visible_profile_facts(text, integer) IS
  'AI assistant read-only Agency model visible profile facts. SECURITY DEFINER with row_security=off and internal guards: auth.uid(), exactly one Agency org membership, and model scope through model_agency_territories before matching. Uses folded exact, full-token, prefix-token, contains, and pg_trgm similarity matching with threshold 0.40. Returns only the best match rank; multiple best-rank candidates are returned for clarification. Max 5 rows. Returns only display name, visible location, measurements, hair/eyes/categories, and account_linked boolean; excludes IDs, email, phone, sync IDs, notes, files, URLs, billing, messages, team, admin/security data, and writes.';
