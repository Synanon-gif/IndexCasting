-- =============================================================================
-- GDPR: export_user_data v4 — Art. 5 minimization on top of v3:
--   invitations: only rows for subject email OR invited_by = subject (no org-wide invitee emails).
--   option_requests / option_request_messages / option_documents: auth UUIDs → *_ref via gdpr_export_actor_ref.
--   client_projects: owner_id → owner_ref.
--   push_tokens: no raw device token (has_token boolean only).
-- anonymize_user_data: clear models.email for rows still linked to subject (Art. 17 residual PII).
-- =============================================================================
...
BEGIN
  ...
  PERFORM public.log_audit_action(
    ...
    p_user_id,
    NULL,
    jsonb_build_object('requested_by', v_uid, 'exported_user', p_user_id, 'export_version', 4),
    NULL,
    'api'
  );
...
BEGIN
  ...
  -- Informational mirror of export_user_data v4 coverage (not enforced at runtime).
...
COMMENT ON FUNCTION public.get_user_related_tables() IS
  'Informational JSON list of user-data tables covered by export_user_data (v4). '
  'Update when export_user_data is extended. Not a security boundary.';

COMMENT ON FUNCTION public.export_user_data(uuid) IS
  'GDPR Art. 15/20 — JSONB export (v4). SECURITY DEFINER + row_security=off. '
  'v3 scope + v4 minimization: invitations limited to subject email / invited_by; option request '
  'participant user UUIDs as *_ref; client_projects owner_ref; push_tokens without raw secret. '
...
-- ─── anonymize_user_data — clear model email for linked model (Art. 17) ─────
...
COMMENT ON FUNCTION public.anonymize_user_data(UUID) IS
  'GDPR anonymization: profile PII, models.email cleared for linked model, org memberships removed, '
  'recruiting messages scrubbed, calendar feed token cleared. Self or admin/super_admin. '
