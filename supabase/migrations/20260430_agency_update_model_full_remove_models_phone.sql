-- =============================================================================
-- agency_update_model_full: remove reference to non-existent models.phone
-- Date: 2026-04-30
--
-- PROBLEM:
--   UPDATE public.models SET ... phone = ... caused PostgreSQL 42703
--   (column "phone" does not exist). public.models has no phone column;
--   see 20260410_security_audit_model_column_revoke.sql (K-1 comment).
--
-- FIX:
--   Drop phone from the UPDATE list. Keep parameter p_phone for API/signature
--   compatibility; value is not persisted (no target column).
--
-- Idempotent: CREATE OR REPLACE. Same signature as 20260429.
-- =============================================================================
