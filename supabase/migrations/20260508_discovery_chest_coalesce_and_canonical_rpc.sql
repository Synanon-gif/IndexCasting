-- =============================================================================
-- Migration: 20260508_discovery_chest_coalesce_and_canonical_rpc.sql
--
-- 1. Canonical get_discovery_models under supabase/migrations/ (was root-only).
-- 2. Chest measurement filters: COALESCE(m.chest, m.bust) matches Client UI
--    (chest ?? bust) for legacy rows with bust-only data.
--
-- Functions touched (logic unchanged except chest predicates + comments):
--   • get_discovery_models
--   • get_models_near_location
--   • get_models_by_location
-- =============================================================================

-- ─── get_discovery_models (v2 body, live-aligned) ───────────────────────────
...
COMMENT ON FUNCTION public.get_discovery_models(
  ...
  'Ranked client discovery (v2). Canonical under migrations 20260508. '
  'Chest min/max use COALESCE(chest,bust) for parity with UI.';

-- ─── get_models_near_location (20260423 body + chest COALESCE) ────────────────
...
COMMENT ON FUNCTION public.get_models_near_location(
  ...
  'DISTINCT ON (model_id) location priority live > current > agency. '
  '20260508: chest filters use COALESCE(chest,bust).';

-- ─── get_models_by_location (20260410 body + chest COALESCE) ─────────────────
...
COMMENT ON FUNCTION public.get_models_by_location(
  ...
  'Paywall via has_platform_access(). 20260508: COALESCE(chest,bust) for chest filters.';
...
BEGIN
  ...
  ), 'get_discovery_models missing after 20260508 migration';

  ASSERT EXISTS (
    SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'get_models_near_location'
      AND 'row_security=off' = ANY(p.proconfig)
  ), 'get_models_near_location must keep row_security=off';

  RAISE NOTICE '20260508_discovery_chest_coalesce_and_canonical_rpc: OK';
END $$;
