-- Fix agency_create_option_request: agency-created events must:
--   1) Set final_status = 'option_confirmed' (agency creating = confirming availability)
--   2) Set model_approval correctly: 'pending' if model has account, 'approved' if not
--   3) Fire calendar triggers by doing INSERT(option_pending) then UPDATE(option_confirmed)
--      - fn_ensure_calendar_on_option_confirmed (AFTER UPDATE) → creates calendar_entries
--      - sync_user_calendars_on_option_confirmed (AFTER INSERT OR UPDATE) → creates user_calendar_events
--   4) Set created_by_agency = true on the calendar_entries row
...
BEGIN
  ...
  -- model_approval: 'pending' if model has app account (needs to confirm),
  -- 'approved' if no account (auto-confirmed per no-account rule)
  ...
  -- STEP 1: INSERT with final_status = 'option_pending' (initial state)
  ...
  -- STEP 2: UPDATE to option_confirmed so AFTER UPDATE triggers fire:
  ...
  -- STEP 3: Mark the calendar entry as agency-created
