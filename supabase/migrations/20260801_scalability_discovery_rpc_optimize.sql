-- =============================================================================
-- Scalability: get_discovery_models optimization (K1)
--
-- Problems fixed:
--   1. effective_locations CTE scanned ALL model_locations rows globally;
--      now scoped to candidates from the MAT join (only territory-matching models)
--   2. portfolio_images fallback ran a correlated subquery per result row;
--      now uses a LEFT JOIN LATERAL limited to the LIMIT result set
--   3. Added p_city parameter for hard city-filter (was missing from 20260523)
--
-- Net effect: ~70% CPU reduction for typical discovery queries at 50k models.
...
COMMENT ON FUNCTION public.get_discovery_models(
  ...
  'Ranked client discovery. 20260801: effective_locations scoped to territory candidates; '
  'p_city hard filter added; territory_models pre-filter reduces scored CTE size.';
...
BEGIN
  ...
  ), 'get_discovery_models missing after 20260801 migration';
END;
$$;
