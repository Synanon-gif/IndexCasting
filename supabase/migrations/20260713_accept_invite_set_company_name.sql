-- =============================================================================
-- 20260713_accept_invite_set_company_name.sql
--
-- PROBLEM: Invited Bookers/Employees have profiles.company_name = NULL because
-- handle_new_user only captures company_name from raw_user_meta_data (which is
-- empty for invite-based signups — the user doesn't enter a company name).
-- The Admin Dashboard then shows an empty "Company Name" field.
--
-- FIX: When accept_organization_invitation succeeds, set profiles.company_name
-- to organizations.name (the org the user is joining). This is safe because the
-- invited user IS joining that org — the org name IS their company.
--
-- Supersedes: 20260611_accept_invite_autofix_role_and_activate.sql
-- =============================================================================
...
COMMENT ON FUNCTION public.accept_organization_invitation(text) IS
  'Secure invitation acceptance with auto-role-fix, auto-activation, and company name sync. '
...
BEGIN
  ...
  ASSERT EXISTS (
    ...
    WHERE proname = 'accept_organization_invitation'
      AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
      AND prokind = 'f'
      AND pg_get_functiondef(oid) ILIKE '%company_name%'
  ), 'FAIL: accept_organization_invitation missing company_name sync';

  RAISE NOTICE 'PASS: accept_organization_invitation — MAJOR-1 + MAJOR-2 + MAJOR-3 fixed';
END $$;
