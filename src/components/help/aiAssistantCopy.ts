...
export function getAiAssistantRoleKnowledge(role: AiAssistantViewerRole): string {
  ...
  if (role === 'client') {
    return [
      ...
      'For Client option or casting requests: use Discover or Projects to open the relevant model/selection, choose Request option or Request casting, select the date/time and request details shown in the form, then send the request. Do not use Agency-only navigation labels or buttons.',
      'Use Projects to organize selections, Messages for agency conversations and negotiation threads, Calendar for visible request or job timing, Team for client organization employees, Billing for the client billing area when available, and Profile/Settings for account or organization details.',
    ].join('\n');
  }
  return [
...
export function getAiAssistantWorkflowGuidance(
  ...
  if (role === 'client' && topic === 'option') {
    return [
      'Use Discover or Projects to open the relevant model or selection.',
