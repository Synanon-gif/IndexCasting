...
function evaluatePlaywrightHarnessSafety(baseUrl, env) {
  ...
  if (prodLike && flags.length > 0) {
    const lines = [
      ...
      '  Fix: use a staging/branch E2E_BASE_URL with disposable data, or a production-readonly profile',
      '       (e.g. .env.e2e.production-readonly from .env.e2e.production-readonly.example; E2E_ENV_FILE=...) ',
